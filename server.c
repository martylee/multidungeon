        ...
        if (select(maxfd + 1, &fdlist, NULL, NULL, NULL) < 0) {
            ...
            for (p = clientlist; p; p = p->next)
                if (FD_ISSET(p->fd, &fdlist))
                    break;
            if (p){
	      if ((flag = (read(p->fd,p->buf,sizeof p->buf))) < 0){
                    perror("read");
	      }else if (flag > 0){
		     p->bytes_in_buf += flag;
                    
                     char *s = memnewline(p->buf,p->bytes_in_buf);
                     if (s){ do_something(p, s);}
...
static void do_say(struct client *p)
{
    ...
    char buf1[200];
    struct client *q;
    strncpy(buf1,p->buf + 4, sizeof(p->buf) -4);//copy the content of the say command
    sprintf(buf, "said %d %s\r\n", p->id, buf1); 
   
    for (q = clientlist; q && p->loc == q->loc ; q = q->next){
      send_string(q->fd, buf);
